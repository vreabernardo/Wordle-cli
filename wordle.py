...
while True:
    ...
    if len(l_input_player) == 5 and (input_player in list_of_words):
        ...
        for x in range(word_lenght):
            ...
            elif list(input_player)[x] != win_word[x] and  list(input_player)[x] in list(win_word) and (list(input_player)[:x+1].count(list(input_player)[x]) <= win_word.count(list(input_player)[x])):
